...
namespace Fanfic.Data
{
    ...
    public class ApplicationDbContext : IdentityDbContext<User>
    {
        ...
        public async Task CreateTale(User user, string name, string shortDescription, TaleGanre ganre, string tags)
        {
            Tale tale = new Tale(user, name, shortDescription, ganre, DateTime.Now);
            if (tags != null)
            {
                foreach (string tag in tags.Split(" ").ToList())
                {
                    Tag newTag = await this.Tags.FindAsync(tag);
                    if (newTag == null)
                    {
                        newTag = new Tag(tag);
                        await this.Tags.AddAsync(newTag);
                    }
                    else
                    {
                        newTag.Weight += 1;
